...
with col2:
    ...
    #.properties(width=650, hight=350) 
...
chart = alt.Chart(df_sta_melted).mark_bar().encode(
        x='시간', y='인원수', color='인원수').properties(width=650, height=350)
text = alt.Chart(df_sta_melted).mark_text(dx=0, dy=0, color='black').encode(
        x='시간', y='인원수', detail='인원수', text=alt.Text('인원수:Q') )
                 # 소수점 이하 1자리: 'sales:Q', format='.1f'
